...
fn strassen_parallel(a: &Matrix, b: &Matrix) -> Matrix {
    ...
    // Izvlačenje pojedinačnih rezultata
    let (m1, m2) = m1_m2;
    let (m3, m4) = m3_m4;
    let (m5, m6) = m5_m6;
    ...
    F: Fn(&Matrix, &Matrix) -> Matrix,
{
    ...
    println!("Vreme izvršavanja: {:.3} sekundi", duration.as_secs_f64());
    println!("Memorija: ~{} MB", mem_used / 1_000_000);
    
    print_matrix_sample(&result, "Rezultat");
}
